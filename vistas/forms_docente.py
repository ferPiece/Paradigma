...
class FormsDocente(tk.PanedWindow):
    ...
    def get_asignatura_etiqueta(self):
        ...
        if not self.__asignatura_etiqueta:
            self.__asignatura_etiqueta = tk.Label(master=self, text="Asignatura: ", width=20)
    ...
    def guardar_docente(self):
        ...
        try:
            ...
            messagebox.showinfo("Registro Docente","Se creo el Docente con exito")
...
if __name__ == '__main__':
    root = tk.Tk()
    forms_docente = FormsDocente(root)
    root.mainloop()     
